...
- [ ] All frontend/workspace checks pass.
- [ ] On the target, move the charger among available ports and verify indexed/physical mapping and sensible voltage/current units.
- [ ] Verify deck/touchpad status where supported without opening the powered laptop or removing internal components for a test.
- [ ] Simulated per-port/deck failures preserve battery and charge-limit functionality and display unavailable/stale status accurately.
